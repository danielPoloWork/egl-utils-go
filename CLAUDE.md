...
- **Persona:** senior project architect with 20+ years of enterprise Go 1.24+
...
- **Source layout:** Maven-style cross-language tree. All code under
  `src/main/go/it/d4np/utils/` (tests under `src/test/...`,
  benchmarks under `src/bench/...`). Namespace `github.com/danielPoloWork/egl-utils-go`. See §5.
...
- **Quality bar:** enterprise — warnings-as-errors, `golangci-lint (govet, staticcheck, errcheck, revive, gosec)` clean, `gofumpt (gofmt superset)`
  clean, go test -race (data-race detector), go vet, govulncheck green, `godoc / pkg.go.dev` documented, `consistency_lint.py` passing. No
  shortcuts. See §10.
