...
func TestHashPasswordDelegatesAtDefaultCost(t *testing.T) {
	...
	require.Equal(t, 12, got,
		"HashPassword is HashPasswordCost at this package's own default cost, not bcrypt's 10")
}

// TestOldDefaultHashesStillVerify pins the property that makes the new default a
// change of policy rather than a data migration: the factor lives in the hash, so
// a store written by v1 — whose default was cost 10 — keeps working untouched. The
// fixture is a literal, captured hash rather than one generated by the code under
// test, so the assertion is about a hash this package did not just produce.
func TestOldDefaultHashesStillVerify(t *testing.T) {
	defer goleak.VerifyNone(t)
	const (
		pw = "correct horse battery staple"
		//nolint:gosec // G101: a bcrypt hash of the test password above, not a credential
		v1Hash = "$2a$10$W9IgPdRBa1v/Gpeo3sApj.a2dfEJTwQ77gOQ3qqr6MJ/JMdltybmW"
	)

	require.NoError(t, hash.CheckPassword(pw, v1Hash), "a v1-era hash must not be invalidated")
	require.ErrorIs(t, hash.CheckPassword("wrong", v1Hash), hash.ErrMismatch)

	got, err := hash.Cost(v1Hash)
	require.NoError(t, err)
	require.Equal(t, 10, got, "it still reports the factor it was made with")
	require.Less(t, got, 12, "which is below the new default, so rehash-on-login has work to do")
}
