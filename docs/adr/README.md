...
| [0024](0024-hash-password-design.md) | hash password design — bcrypt at default cost, per-hash salt, constant-time verify | Accepted (the *value* of the default cost superseded by ADR-0052: cost 12 from `/v2`, this module's own constant; every other decision stands) |
...
| [0040](0040-contrib-submodules.md) | contrib/* nested submodules — require the released core, no replace/workspace; the module boundary is the enforcement | Accepted (amended 2026-07-30: the released core is now `/v2 v2.0.0` — nothing superseded, and its "released core" rule is what scheduled roadmap 13.9 after the release) |
...
| [0052](0052-hash-default-cost-12.md) | bcrypt's default work factor becomes 12 — the module owns its default instead of inheriting `bcrypt.DefaultCost`; the strong value is what you get by not choosing, and cost 10 stays legal but has to be asked for | Accepted |
| [0053](0053-runnable-examples-convention.md) | runnable examples are documentation that runs — external test package, a verified `// Output:` (an example without one is compiled and never run), printed *shape* rather than strings, and no example that depends on the clock | Accepted |
| [0054](0054-examples-service-module.md) | `examples/service` — the cross-package composition an `Example` cannot show, as its own module: the released core with no `replace`, the nested-module boundary now enforced recursively, and no coverage floor, because `main()` is a third of the statements and none are reachable | Accepted |
| [0055](0055-contrib-release-workflow.md) | verifying a `contrib/*` release on its tag — the parse from tag name to module directory *is* the job, reachability from `master` is how the tag inherits the CI gate, and no GitHub Release is drafted, because an unpublished tag is a recoverable mistake and a published Release is not | Accepted |
| [0056](0056-build-time-supply-chain.md) | the build-time supply chain — every action pinned to a digest with the version in a comment, no workflow-level token grant so a future job cannot inherit one, a reproducible CycloneDX SBOM as the project's first release artifact with a provenance claim scoped to that document rather than to the module (the checksum database already answers that), gated in `consistency_lint.py` rather than a fifth tool, and signed tags declined while `master`'s commits turn out to be signed already | Accepted |
| [0057](0057-additive-capability-ledger.md) | the additive-capability ledger — every deferred capability gets an entry with a **trigger**, the evidence that would schedule it, because a table without one is a backlog and a backlog invites the speculative surface each of those ADRs refused; the census found 49 open capabilities across 26 ADRs rather than the roadmap's "twenty across sixteen", and eleven already-discharged deferrals whose ADRs never said so | Accepted |
| [0058](0058-no-documentation-site.md) | no documentation site — `pkg.go.dev` already renders 55 verified examples for the audience that wants one, while Pages from `docs/` would publish only `docs/` and so 404 on every link to `AGENTS.md`, `CONTRIBUTING.md` and `CODE_OF_CONDUCT.md`, which are root files by convention; a curated site is registered as a deferred capability rather than dismissed | Accepted |
