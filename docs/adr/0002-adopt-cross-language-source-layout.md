...
`egl-utils-go` is one of a family of projects intended to share the same technical-
...
For this repository `<lang>` = `go` and the namespace/package is `github.com/danielPoloWork/egl-utils-go`,
...
- Build tooling is configured to treat `src/main/go/...` as the source root; some
