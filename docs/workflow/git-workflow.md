...
Full conventions for branches, commits, and pull requests on `egl-utils-go`. The short
...
- `scope` is a short subsystem noun. Scopes for this repo: `concurrency` `resilience` `middleware` `config` `logging` `storage` `validation` `lifecycle` `api` `build` `tests` `docs` `ci` .
