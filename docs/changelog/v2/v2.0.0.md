...
The module's second major. Feature packages moved under `pkg/`, the module path gained its `/v2`
suffix, and the seven breaking changes deferred at v1.0.0 were all discharged in this one release —
**every consumer import changes, and seven APIs change with them**.

The mechanical half of the migration is a path rewrite:
`…/egl-utils-go/<pkg>` → `…/egl-utils-go/v2/pkg/<pkg>`. The rest is listed below, in the order the
work landed; [the release notes](../../releases/v2.0.0.md) carry the full per-package migration table.
...
- **BREAKING** — feature packages moved from the module root to `pkg/`, and the module path is now
  ...
  ([ADR-0045](../../adr/0045-pkg-layout-and-v2.md), supersedes ADR-0003). Module metadata
...
- **BREAKING** — the `errors` package is now **`errx`** (`…/v2/pkg/errx`), and **stack capture is
  opt-in** ([ADR-0046](../../adr/0046-errx-opt-in-stacks.md), supersedes ADR-0029). `Wrap`/`Wrapf`
...
- **BREAKING** — `cache.Get` now returns **`(V, bool)`** instead of `(V, error)`, `NewInMemory` is
  ...
  ([ADR-0047](../../adr/0047-cache-comma-ok.md), supersedes ADR-0021's `Get` signature and
...
- **BREAKING** — `workerpool.Stop` is now **`Close`** and `ErrPoolClosed` is now **`ErrClosed`**
  ([ADR-0048](../../adr/0048-workerpool-close.md), supersedes those two names in ADR-0005 and nothing
...
- **BREAKING** — the **pubsub API is reshaped**: subscriptions are context-scoped, `Publish` takes a
  ...
  ([ADR-0049](../../adr/0049-pubsub-reshape.md), supersedes the two signatures and the fixed
...
- **BREAKING** — **`metrics` no longer depends on the Prometheus SDK**; it writes the text exposition
  ...
  ([ADR-0050](../../adr/0050-metrics-without-the-sdk.md), supersedes ADR-0027's surface,
  ...
  **What is given up, deliberately: the endpoint no longer serves the 37 metric families `promhttp`
  ...
  [the report](../../benchmarks/2026-07-29-metrics-without-the-sdk.md).
- **BREAKING** — **`lifecycle.WaitForSignals` takes a shutdown timeout as its first argument**
  ([ADR-0051](../../adr/0051-lifecycle-shutdown-timeout.md), supersedes ADR-0025's "no hidden timeout"
...
- **BREAKING (behaviour, not signature)** — **`hash.HashPassword` now produces bcrypt cost 12**, up
  from bcrypt's default of 10 ([ADR-0052](../../adr/0052-hash-default-cost-12.md), supersedes the
