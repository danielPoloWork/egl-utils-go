...
| C-2 | Untrusted HTTP input handling — client-supplied header values are sanitized (bounded length, visible-ASCII only) before being echoed, propagated, or logged; identifiers used for correlation are generated with a CSPRNG and never treated as identity | [ADR-0013](../adr/0013-middleware-requestid-design.md) | `middleware` sanitizer + `isValidID`/property tests · [threat model](../security/threat-model.md) Tampering/Info-disclosure rows · CI `quality` (`gosec`, `go test`) | Active |
