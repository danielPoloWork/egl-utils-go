...
| C-4 | Password hashing at rest — passwords are stored only as salted, adaptive bcrypt hashes at cost ≥ 10 (per-hash random salt embedded in the hash); verification is constant-time; over-long input is rejected, not truncated; plaintext is never logged or persisted by the `hash` package | [ADR-0024](../adr/0024-hash-password-design.md) | `hash` round-trip / salt / mismatch / too-long tests · cost-10 prefix assertion (`TestHashUsesDefaultCost`) · `govulncheck` (x/crypto) · security-auditor sign-off | Active |
