...
- **Session journal:** see [`docs/journal/`](docs/journal/). Latest checkpoint:
  [2026-07-26 — M10.6: config.WithStructValidation()](docs/journal/2026/07/2026-07-26-m10-config-struct-validation.md).
...
- [ ] 10.8 Import-graph enforcement — depguard rules for the ADR-0004 allowlist (yaml.v3→config only, prometheus→metrics only) + go mod graph CI assertion (v2 §3) — *agent: Opus 4.8 · medium — **must explicitly permit the `config → validator` same-layer edge 10.6 established ([ADR-0033](docs/adr/0033-config-struct-validation.md)) or the build breaks; the rule to express is "same-layer edges only where the spec mandates the composition", not "L2 is a free-for-all"***
