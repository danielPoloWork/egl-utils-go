...
- **Session journal:** see [`docs/journal/`](docs/journal/). Latest checkpoint:
  [2026-07-27 — M13 opens: `/v2`, `pkg/`, and a layout decided by building both](docs/journal/2026/07/2026-07-27-m13-v2-pkg-layout.md).
...
- [ ] 13.2 `errors` → `errx`, with opt-in `WithStack` / `[]Frame` (ledger, v2 item 25) — *agent: Opus 5 · high — the rename is the easy half; the design question is whether stack capture becomes opt-in and what `[]Frame` exposes without leaking `runtime` types into the surface. ADR-0029's one-time-origin-stack behaviour is the baseline to preserve or consciously drop*
